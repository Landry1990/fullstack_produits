...
deploy_backend() {
    ...
    warn "  Copie des dossiers de code (api, backend, scripts)..."
    docker cp backend/manage.py fullstack_produits-backend-1:/app/manage.py
    docker cp backend/api/. fullstack_produits-backend-1:/app/api/
    docker cp backend/backend/. fullstack_produits-backend-1:/app/backend/
    if [[ -d "backend/scripts" ]]; then
        docker cp backend/scripts/. fullstack_produits-backend-1:/app/scripts/
    fi
