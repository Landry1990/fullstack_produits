...
def _write_sheet_header(ws, pharmacy, titre: str, logo_path: str | None = None, lang: str | None = None):
    ...
    ws.cell(row=3, column=col,
            value=f"{T(lang, 'doc_tel', v=pharmacy.get('phone', ''))}  |  {pharmacy.get('email', '')}".strip(" |"))
    ...
    ws.cell(row=5, column=col,
            value=T(lang, 'doc_edited', d=format_doc_date(now, lang, with_time=True)))
...
def _sheet_synthese(wb, data: dict, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_synthese'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_synthese', mois=mois_label), logo_path, lang=lang)

    ca     = data["ca"]
    marge  = data["marge"]
    divers = data["divers_ca"]

    # (clé i18n, valeur, genre) — genre pilote le formatage, indépendant du libellé
    rows = [
        ('xs_r_ca_pharma_ttc',  ca["ca_ttc"] - divers["ca_ttc"],                 'money'),
        ('xs_r_ca_divers_ttc',  divers["ca_ttc"],                                'money'),
        ('xs_r_ca_total_ttc',   ca["ca_ttc"],                                    'money_total'),
        ('', "", 'blank'),
        ('xs_r_marge_pharma',   marge["marge_pharma"],                           'money'),
        ('xs_r_taux_pharma',    None,                                            'pct_pharma'),
        ('xs_r_marge_divers',   marge["marge_divers"],                           'money'),
        ('xs_r_marge_totale',   marge["marge_brute"],                            'money_total'),
        ('xs_r_taux_total',     None,                                            'pct_total'),
        ('', "", 'blank'),
        ('xs_r_remises',        ca.get("total_remises", 0),                      'money'),
        ('xs_r_rem_glob',       ca.get("total_remises_detail", {}).get("global", 0),   'money'),
        ('xs_r_rem_lignes',     ca.get("total_remises_detail", {}).get("lignes", 0),   'money'),
        ('xs_r_rem_fid',        ca.get("total_remises_detail", {}).get("fidelite", 0), 'money'),
        ('', "", 'blank'),
        ('xs_r_nb_ventes',      ca.get("nb_ventes", 0),                          'int'),
        ('xs_r_panier',         (ca["ca_ttc"] / ca["nb_ventes"]) if ca.get("nb_ventes") else 0, 'money'),
        ('', "", 'blank'),
        ('xs_r_creances',       data["creances"]["total"],                       'money'),
        ('xs_r_dettes',         data.get("dettes_total", 0),                     'money'),
    ]
    ...
    _write_col_headers(ws, start, [T(lang, 'xs_col_indicateur'), T(lang, 'xs_col_montant_valeur')])
    r = start + 1
    for i, (key, val, kind) in enumerate(rows):
        if not key:
            r += 1
            continue
        is_total = kind == 'money_total'
...
def _sheet_ca_marges(wb, daily_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_ca_marges'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_ca', mois=mois_label), logo_path, lang=lang)

    headers = [
        T(lang, 'col_date'), T(lang, 'kpi_nb_ventes'),
        T(lang, 'xsc_ca_pharma'), T(lang, 'xsc_ca_divers'), T(lang, 'xsc_ca_total'),
        T(lang, 'xsc_marge_pharma'), T(lang, 'xsc_marge_divers'), T(lang, 'xsc_marge_totale'), T(lang, 'col_taux_marge'),
        T(lang, 'kpi_remises')
    ]
    ...
    total_vals = [
        T(lang, 'row_total'), int(totals["nb_ventes"]),
...
def _sheet_remises_avoirs(wb, remises_rows: list, avoirs_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_remises'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_remises', mois=mois_label), logo_path, lang=lang)

    # ── Remises ──
    ws.cell(row=start, column=1, value=T(lang, 'xs_sec_remises')).font = _font(bold=True, size=12, color=PALETTE["sub_bg"])
    start += 1
    h_remises = [T(lang, 'col_date'), T(lang, 'col_num_facture'), T(lang, 'col_client'), T(lang, 'xcol_remise_globale'), T(lang, 'xcol_remise_lignes'), T(lang, 'xcol_fidelite'), T(lang, 'xcol_total_remise')]
    ...
    for i, row in enumerate(remises_rows):
        ...
        r += 1
    # Total remises
    for c, v in enumerate([T(lang, 'row_total'), "", "", "", "", "", _fmt(total_rem)], 1):
        ws.cell(row=r, column=c, value=v)
    _row_style(ws, r, len(h_remises), is_total=True)
    r += 2

    # ── Avoirs ──
    ws.cell(row=r, column=1, value=T(lang, 'xs_sec_avoirs')).font = _font(bold=True, size=12, color=PALETTE["sub_bg"])
    r += 1
    h_avoirs = [T(lang, 'col_date'), T(lang, 'col_num_avoir'), T(lang, 'col_fournisseur'), T(lang, 'col_montant_ht'), T(lang, 'col_statut')]
    ...
    for i, row in enumerate(avoirs_rows):
        ...
        r += 1
    for c, v in enumerate([T(lang, 'row_total'), "", "", _fmt(total_av), ""], 1):
        ws.cell(row=r, column=c, value=v)
...
def _sheet_top_produits(wb, top_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_top'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_top', mois=mois_label), logo_path, lang=lang)

    headers = [T(lang, 'col_rang'), T(lang, 'col_produit'), "CIP", T(lang, 'col_rayon'), T(lang, 'col_type'), T(lang, 'col_qte_vendue'), T(lang, 'col_ca_ttc_f'), T(lang, 'col_cout_achat'), T(lang, 'col_marge_f'), T(lang, 'col_taux_marge')]
    _write_col_headers(ws, start, headers)
    r = start + 1
    for i, row in enumerate(top_rows):
        vals = [
            ...
            row.get("cip", ""),
            row.get("rayon", ""),
            T(lang, 'val_divers') if row.get("is_divers") else T(lang, 'val_pharmacie'),
            row.get("quantite", 0),
...
def _sheet_dettes_fournisseurs(wb, dettes_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_dettes'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_dettes', mois=mois_label), logo_path, lang=lang)

    headers = [T(lang, 'col_fournisseur'), T(lang, 'col_total_commande'), T(lang, 'col_total_paye'), T(lang, 'col_solde_du'), T(lang, 'col_echu'), T(lang, 'col_a_venir'), T(lang, 'col_prochaine_ech')]
    _write_col_headers(ws, start, headers)
    r = start + 1
    totals = defaultdict(Decimal)
    for i, row in enumerate(dettes_rows):
        ...
        r += 1
    for c, v in enumerate([T(lang, 'row_total'), _fmt(totals["total_commande"]), _fmt(totals["total_paye"]),
...
def _sheet_creances_clients(wb, creances_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_creances'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_creances', mois=mois_label), logo_path, lang=lang)

    headers = [T(lang, 'col_client'), T(lang, 'col_type'), T(lang, 'col_nb_factures'), T(lang, 'col_montant_total'), T(lang, 'col_paye_f'), T(lang, 'col_restant_du'), T(lang, 'col_jours_retard')]
    _write_col_headers(ws, start, headers)
    r = start + 1
    totals = defaultdict(Decimal)
    for i, row in enumerate(creances_rows):
        ...
        r += 1
    for c, v in enumerate([T(lang, 'row_total'), "", "", _fmt(totals["ca_total"]), _fmt(totals["montant_paye"]), _fmt(totals["reste_a_payer"]), ""], 1):
        ws.cell(row=r, column=c, value=v)
    _row_style(ws, r, len(headers), is_total=True)
    _auto_width(ws)
...
def _sheet_stock(wb, stock_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_stock'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_stock', mois=mois_label), logo_path, lang=lang)

    headers = [T(lang, 'col_rayon'), T(lang, 'col_nb_refs'), T(lang, 'col_qte_totale'), T(lang, 'col_valeur_pmp'), T(lang, 'col_stock_min'), T(lang, 'col_alertes_rupture'), T(lang, 'col_alertes_peremption')]
    _write_col_headers(ws, start, headers)
    r = start + 1
    totals = defaultdict(Decimal)
    for i, row in enumerate(stock_rows):
        ...
        r += 1
    for c, v in enumerate([T(lang, 'row_total'), int(totals["nb_refs"]), int(totals["qte_totale"]),
...
def _sheet_fiscal(wb, data: dict, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    ...
    ws = wb.create_sheet(T(lang, 'xs_sheet_fiscal'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_fiscal', mois=mois_label), logo_path, lang=lang)
    ...
    ws.cell(row=r, column=1, value=T(lang, 'fis_regime')).font = _font(bold=True)
    ws.cell(row=r, column=2, value=regime_label).alignment = _align(h="right")
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'fis_mode')).font = _font(bold=True)
    ws.cell(row=r, column=2, value=mode_label).alignment = _align(h="right")
    r += 2

    # Section : Chiffre d'affaires
    ca_data = data.get("ca", {})
    ws.cell(row=r, column=1, value=T(lang, 'fis_ca')).font = _font(bold=True, size=11)
    r += 1
    for label, key in [(T(lang, 'kpi_ca_ht'), "ca_ht"), (T(lang, 'kpi_ca_ttc'), "ca_ttc"), (T(lang, 'fis_tva'), "tva_collectee")]:
    ...
    ws.cell(row=r, column=1, value=T(lang, 'fis_achats')).font = _font(bold=True, size=11)
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'fis_total_achats'))
    ...
    if mode == 'DROIT_COMMUN':
        ...
        ws.cell(row=r, column=1, value=T(lang, 'fis_taux_precompte', regime=regime_label))
        ws.cell(row=r, column=2, value=f"{taux_prec}%").alignment = _align(h="right")
        r += 1
        ws.cell(row=r, column=1, value=T(lang, 'fis_total_precompte')).font = _font(bold=True)
        ws.cell(row=r, column=2, value=_fmt(achats_data.get("total_precompte", 0))).alignment = _align(h="right")
        r += 1
    else:
        ws.cell(row=r, column=1, value=T(lang, 'fis_precompte_na'))
    ...
    ws.cell(row=r, column=1, value=T(lang, 'fis_accompte')).font = _font(bold=True, size=11)
    r += 1
    ws.cell(row=r, column=1, value=accompte_data.get("base_label") or T(lang, 'fis_base_ca'))
    ws.cell(row=r, column=2, value=_fmt(accompte_data.get("base_imposition", 0))).alignment = _align(h="right")
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'col_taux'))
    ws.cell(row=r, column=2, value=f"{accompte_data.get('taux', 0)}%").alignment = _align(h="right")
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'fis_accompte_base'))
    ws.cell(row=r, column=2, value=_fmt(accompte_data.get("accompte_base", 0))).alignment = _align(h="right")
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'fis_cac', t=accompte_data.get('taux_cac', 0)))
    ws.cell(row=r, column=2, value=_fmt(accompte_data.get("accompte_cac", 0))).alignment = _align(h="right")
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'fis_accompte_total')).font = _font(bold=True)
...
def _sheet_ugs(wb, data: dict, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    ...
    ws = wb.create_sheet(T(lang, 'xs_sheet_ugs'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_ugs', mois=mois_label), logo_path, lang=lang)

    headers = [T(lang, 'col_produit'), T(lang, 'col_fournisseur'), T(lang, 'col_ug_recues'), T(lang, 'col_ug_vendues'), T(lang, 'col_ug_restantes'), T(lang, 'col_lot_num')]
    _write_col_headers(ws, start, headers)
    r = start + 1
    ...
    # Ligne totale
    for c, v in enumerate([T(lang, 'row_total'), "", totals["ug_recues"], totals["ug_vendues"],
    ...
    # Section résumé
    ws.cell(row=r, column=1, value=T(lang, 'xs_sec_resume')).font = _font(bold=True, size=11)
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'ug_total_recues'))
    ws.cell(row=r, column=2, value=totals["ug_recues"]).alignment = _align(h="right")
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'ug_total_vendues'))
    ws.cell(row=r, column=2, value=totals["ug_vendues"]).alignment = _align(h="right")
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'ug_total_restantes'))
    ws.cell(row=r, column=2, value=totals["ug_restantes"]).alignment = _align(h="right")
    r += 1
    ws.cell(row=r, column=1, value=T(lang, 'ug_taux_rotation'))
...
def _sheet_achats(wb, achats_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_achats'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_achats', mois=mois_label), logo_path, lang=lang)

    headers = [T(lang, 'col_fournisseur'), T(lang, 'col_nb_commandes'), T(lang, 'col_montant_ht'), T(lang, 'col_precompte'), T(lang, 'col_nb_avoirs'), T(lang, 'col_montant_avoirs'), T(lang, 'col_net')]
    _write_col_headers(ws, start, headers)
    r = start + 1
    totals = defaultdict(Decimal)
    for i, row in enumerate(achats_rows):
        ...
        r += 1
    for c, v in enumerate([T(lang, 'row_total'), "", _fmt(totals["montant_total"] + totals["montant_avoirs"]),
...
def _write_clotures_table(ws, rows: list, start_row: int, lang: str | None = None) -> int:
    ...
    headers = [
        T(lang, 'col_date'), T(lang, 'col_caisse_vendeur'), T(lang, 'col_heure_ouv'), T(lang, 'col_heure_ferm'),
        T(lang, 'col_ventes_f'), T(lang, 'col_entrees_f'), T(lang, 'col_sorties_f'),
        T(lang, 'col_montant_theo'), T(lang, 'col_montant_reel'), T(lang, 'col_ecart_f'), T(lang, 'col_observation')
    ]
...
def _sheet_etat_caisses(wb, clotures_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_caisses'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_caisses', mois=mois_label), logo_path, lang=lang)

    # ── Tableau global ────────────────────────────────────────────────────────
    cell = ws.cell(row=start, column=1, value=T(lang, 'xs_sec_recap'))
    cell.font = _font(bold=True, size=12, color=PALETTE["sub_bg"])
    r = _write_clotures_table(ws, clotures_rows, start + 1, lang=lang)

    # ── Sous-tableaux par caissier ────────────────────────────────────────────
    # Regrouper par caissier (en conservant l'ordre d'apparition)
    caissiers: dict = {}
    for row in clotures_rows:
        nom = row.get("caissier") or T(lang, 'user_unknown')
    ...
    if len(caissiers) > 1:
        ...
        for nom, rows in caissiers.items():
            cell = ws.cell(row=r, column=1, value=T(lang, 'xs_caissier', nom=nom.upper()))
            cell.font = _font(bold=True, size=11, color=PALETTE["header_bg"])
            cell.fill = _fill(PALETTE["alt_row"])
            r += 1
            r = _write_clotures_table(ws, rows, r, lang=lang)
...
def _sheet_depenses(wb, depenses_rows: list, pharmacy: dict, mois_label: str, logo_path, lang: str | None = None):
    from api.utils_doclang import T, format_doc_date, get_document_language

    if not lang:
        lang = get_document_language()

    ws = wb.create_sheet(T(lang, 'xs_sheet_depenses'))
    start = _write_sheet_header(ws, pharmacy, T(lang, 'xs_t_depenses', mois=mois_label), logo_path, lang=lang)

    headers = [T(lang, 'col_date'), T(lang, 'col_heure'), T(lang, 'col_caisse'), T(lang, 'col_type'), T(lang, 'col_motif'), T(lang, 'col_description'), T(lang, 'col_montant_f'), T(lang, 'col_saisi_par')]
    _write_col_headers(ws, start, headers)
    r = start + 1
    ...
    for i, row in enumerate(depenses_rows):
        ...
        dt = row.get("date")
        date_str = format_doc_date(dt, lang) if isinstance(dt, (datetime, date)) else str(dt or "")
    ...
    for label, val, color in [
        (T(lang, 'dep_total_sorties'), total_sorties, PALETTE["neg"]),
        (T(lang, 'dep_total_entrees'), total_entrees, PALETTE["pos"]),
        (T(lang, 'dep_solde'), total_entrees - total_sorties, PALETTE["header_bg"]),
    ]:
...
def _compute_accompte_fiscal_inline(ca_ht, factures_qs, lang: str | None = None):
    ...
    if mode == 'MARGE_ADMINISTREE':
        ...
        base_label = T(lang, 'fis_base_marge')
    else:
        if regime == 'REEL':
...
def _collect_data(date_debut, date_fin, lang: str | None = None):
    ...
    for f in factures.select_related("client").prefetch_related("produits"):
        ...
        if total_rem_f > 0:
            remises_rows.append({
                "date":           format_doc_date(f.date, lang),
    ...
    for a in Avoir.objects.filter(
        ...
        avoirs_rows.append({
            "date":        format_doc_date(a.date, lang) if a.date else "",
    ...
    for rayon in rayons_list:
        ...
        stock_rows.append({
            "rayon":              rayon.name if rayon else T(lang, 'grp_sans_rayon_paren'),
            "nb_refs":            nb_refs,
    ...
    for cl in ClotureCaisse.objects.filter(
        ...
        if cl.user:
            ...
            caissier = T(lang, 'user_unknown')
        clotures_rows.append({
            "date":             format_doc_date(dt, lang),
            "caisse":           (cl.poste_caisse.nom if cl.poste_caisse else T(lang, 'col_caisse')) +
    ...
    for m in MouvementCaisse.objects.filter(
        ...
        depenses_rows.append({
            ...
            "user":        m.user.get_full_name() if m.user else T(lang, 'user_unknown'),
        })
    ...
    return {
        ...
        "achats_fiscaux": {
            "total_achats_ht": int(sum((Decimal(str(r.get("montant_total", 0) or 0)) for r in achats_rows), Decimal('0')).quantize(Decimal(1), rounding=ROUND_HALF_UP)),
            "total_precompte": int(total_precompte_excel.quantize(Decimal(1), rounding=ROUND_HALF_UP)) if total_precompte_excel else 0,
        },
        "accompte_fiscal": _compute_accompte_fiscal_inline(agg["ca_total_ht"], factures_ann, lang=lang),
    }
...
def build_rapport_general_excel(date_debut, date_fin, mois_label: str, lang: str | None = None) -> HttpResponse:
    """
    ...
    from api.utils_doclang import get_document_language
    ...
    data  = _collect_data(date_debut, date_fin, lang=lang)
    extra = collect_extra_data(date_debut, date_fin, lang=lang)
    ...
    ws_synthese = _sheet_synthese(wb, data, pharmacy, mois_label, logo_path, lang=lang)
    # Enrichir la Synthèse avec évolution + objectif
    enrich_synthese(ws_synthese, extra, lang=lang)

    _sheet_ca_marges(wb, data["daily_rows"], pharmacy, mois_label, logo_path, lang=lang)
    _sheet_remises_avoirs(wb, data["remises_rows"], data["avoirs_rows"], pharmacy, mois_label, logo_path, lang=lang)
    _sheet_top_produits(wb, data["top_rows"], pharmacy, mois_label, logo_path, lang=lang)
    _sheet_dettes_fournisseurs(wb, data["dettes_rows"], pharmacy, mois_label, logo_path, lang=lang)
    _sheet_creances_clients(wb, data["creances_rows"], pharmacy, mois_label, logo_path, lang=lang)
    _sheet_stock(wb, data["stock_rows"], pharmacy, mois_label, logo_path, lang=lang)
    _sheet_achats(wb, data["achats_rows"], pharmacy, mois_label, logo_path, lang=lang)
    _sheet_etat_caisses(wb, data["clotures_rows"], pharmacy, mois_label, logo_path, lang=lang)
    _sheet_depenses(wb, data["depenses_rows"], pharmacy, mois_label, logo_path, lang=lang)

    # ── Feuille fiscale ──
    _sheet_fiscal(wb, data, pharmacy, mois_label, logo_path, lang=lang)

    # ── Feuille UGs ──
    _sheet_ugs(wb, data, pharmacy, mois_label, logo_path, lang=lang)

    # ── Nouvelles feuilles (11-17) ──
    sheet_modes_paiement(wb, extra, pharmacy, mois_label, logo_path, lang=lang)
    sheet_retours_annulations(wb, extra, pharmacy, mois_label, logo_path, lang=lang)
    sheet_performance_vendeurs(wb, extra, pharmacy, mois_label, logo_path, lang=lang)
    sheet_tresorerie(wb, extra, pharmacy, mois_label, logo_path, lang=lang)
    sheet_perimes(wb, extra, pharmacy, mois_label, logo_path, lang=lang)
    sheet_promotions(wb, extra, pharmacy, mois_label, logo_path, lang=lang)
    sheet_clients_pro(wb, extra, pharmacy, mois_label, logo_path, lang=lang)
