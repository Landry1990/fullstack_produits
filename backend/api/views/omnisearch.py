...
from ..serializers_optimized import (
    ...
    FactureListSerializer, CommandeListSerializer
)
...
class GlobalSearchView(APIView):
    ...
    def get(self, request):
        ...
        ).select_related('client', 'created_by', 'validated_by', 'ayant_droit')[:limit]

        # 4. COMMANDES
        # On doit annoter pour CommandeListSerializer
        commandes = Commande.objects.filter(
            Q(numero_facture__icontains=query) |
            Q(fournisseur__name__icontains=query) |
            Q(fournisseur_nom__icontains=query)
        ).select_related('fournisseur', 'closed_by').annotate(
        ...
        return Response({
            ...
            'factures': FactureListSerializer(factures, many=True).data,
            'commandes': CommandeListSerializer(commandes, many=True).data,
