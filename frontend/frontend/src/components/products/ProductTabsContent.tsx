...
const PriceEvolutionChart = ({ achats, t }: { achats: any[]; t: any }) => {
    ...
    return (
        <div className="mb-4 bg-gray-50 rounded-2xl border border-gray-100 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                <div className="flex items-center gap-2">
                    <span className="text-base font-black uppercase tracking-wider text-gray-500">
                        📈 {t('products:detail.purchases.price_evolution', { defaultValue: 'Évolution Prix Achat' })}
                    </span>
                    {hasMultiplePoints && (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold ${
                            isStable ? 'bg-gray-100 text-gray-500' : isHausse ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'
                        }`}>
                ...
                {fournisseurs.length > 1 && (
                    <select
                        className="h-8 px-3 text-xs font-semibold border border-gray-200 rounded-lg bg-white text-gray-700 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-50 outline-none"
            ...
            <div className="flex gap-4 mb-3 text-xs">
                <span className="font-bold text-gray-400">
                    Min : <span className="text-emerald-600 font-black">{formatCurrency(minPrix)}</span>
                </span>
                <span className="font-bold text-gray-400">
                    Max : <span className="text-red-600 font-black">{formatCurrency(maxPrix)}</span>
                </span>
                <span className="font-bold text-gray-400">
...
const PurchasesTabContent = ({ achats, t }: { achats: any[]; t: any }) => {
    if (!achats || achats.length === 0) return <p className="text-center text-gray-400 py-8">{t('products:detail.purchases.empty')}</p>;
    ...
    return (
        <div>
            ...
            <table className="table">
                ...
                <tbody>
                    {achats.map((achat) => (
                        <tr key={achat.id} className="hover:bg-gray-50 transition-colors border-b border-gray-50">
                            <td className="text-sm font-mono font-bold text-gray-500">{formatDate(achat.commande_date)}</td>
                            ...
                            <td>
                                <span className="inline-flex items-center px-2 py-0.5 rounded border border-gray-200 text-xs font-mono font-semibold text-gray-600 bg-gray-50">{achat.lot || '-'}</span>
                            </td>
                            <td className="text-sm font-bold text-gray-400">
...
const LotsTabContent = ({ lots, t }: { lots: StockLot[]; t: any }) => {
    if (!lots || lots.length === 0) return <p className="text-center text-gray-400 py-8">{t('products:detail.lots.empty')}</p>;
            ...
            <table className="table">
                ...
                <tbody>
                    {lots.map((lot) => {
                        ...
                        return (
                            <tr key={lot.id} className="hover:bg-gray-50 transition-colors border-b border-gray-50">
                                <td className="text-sm font-mono font-bold text-gray-500">{formatDate(lot.date_reception)}</td>
                                <td>
                                    <span className="inline-flex items-center px-2 py-0.5 rounded border border-gray-200 text-xs font-mono font-bold text-gray-700 bg-gray-50">{lot.lot || '-'}</span>
                                </td>
                                <td>
                                    <span className={`text-sm font-black ${isExpired ? 'text-red-600' : 'text-gray-700'}`}>
                                ...
                                <td className="text-right font-black text-sm">
                                    <span className={lot.quantity_remaining > 0 ? 'text-emerald-600' : 'text-gray-300'}>
...
const StatsTabContent = ({ monthlyStats, t }: { monthlyStats: any[]; t: any }) => {
    if (!monthlyStats || monthlyStats.length === 0) return <p className="text-center text-gray-400 py-4">{t('products:detail.stats.empty')}</p>;
        ...
        <div className="overflow-x-auto max-h-80">
            <table className="table w-full">
                <thead className="bg-white sticky top-0 border-b border-gray-200">
                    ...
                    <th className="text-[11px] font-black uppercase text-right text-indigo-600 whitespace-nowrap">{t('products:detail.stats.qty_sold')}</th>
                    <th className="text-[11px] font-black uppercase text-right text-amber-600 whitespace-nowrap">{t('products:detail.stats.qty_ordered')}</th>
                    <th className="text-[11px] font-black uppercase text-right text-blue-600 whitespace-nowrap">{t('products:detail.stats.nb_clients')}</th>
                    </tr>
                </thead>
                <tbody>
                    {(monthlyStats || []).map((stat, index) => {
                        ...
                        return (
                            <tr key={index} className={`border-b border-gray-50 ${showYear ? 'border-t-2 border-gray-200' : ''}`}>
                                <td className="font-black text-sm text-gray-400">
                                    {showYear ? stat.year : ''}
                                </td>
                                <td className="text-sm font-bold">{stat.month_name}</td>
                                <td className="text-right font-mono font-black text-sm text-indigo-600">{stat.qte_v}</td>
                                <td className="text-right font-mono font-bold text-sm text-amber-600">{stat.qte_c}</td>
                                <td className="text-right font-mono font-bold text-sm text-blue-600">{stat.nb_c}</td>
                            </tr>
                        );
                    })}
                </tbody>
            </table>
            <div className="mt-2 text-[10px] text-gray-400 flex justify-around">
...
const MovementsTabContent = ({ stockHistory, loadingHistory, onMovementClick, t }: { stockHistory: any[]; loadingHistory: boolean; onMovementClick: (item: any) => void; t: any }) => {
    ...
    if (!stockHistory || stockHistory.length === 0) return <p className="text-center text-gray-400 py-8">{t('products:detail.movements.empty')}</p>;
                ...
                <tbody>
                    {(stockHistory || []).map((item, index) => {
                        ...
                        return (
                            <tr 
                                key={index} 
                                className={`hover:bg-gray-50 transition-colors border-b border-gray-50 ${(item.facture || item.commande) ? 'cursor-pointer' : ''}`}
                                onClick={() => onMovementClick(item)}
                            >
                                <td className="whitespace-nowrap text-sm font-mono font-bold text-gray-500">
                                ...
                                <td className="text-sm font-bold" title={item.libelle}>
                                    <div className="flex items-center gap-1">
                                        ...
                                        {item.commande_numero && (
                                            <span className="inline-flex items-center px-2 py-0.5 rounded border border-gray-200 text-xs font-mono font-bold text-gray-500 bg-gray-50 ml-auto">
                                                {item.commande_numero}
                                            </span>
                                        )}
                                    </div>
                                </td>
                                <td className="text-sm font-bold text-gray-400">{item.user || item.user_nom || '-'}</td>
                                <td className="text-right font-mono text-sm font-bold text-gray-400">{item.stock_avant}</td>
                                <td className={`text-right font-black text-sm ${isPositive ? 'text-emerald-600' : 'text-red-600'}`}>
...
export const ProductTabsContent: React.FC<ProductTabsContentProps> = ({
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Onglets */}
      <div className="bg-white shrink-0 border-b border-gray-200">
        ...
        {[
          ...
          <button
            key={tab.id}
            className={`px-3 py-1.5 text-xs font-semibold uppercase tracking-tight rounded-lg transition-colors whitespace-nowrap ${activeTab === tab.id ? 'bg-indigo-50 text-indigo-700' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'}`}
            ...
            <table className="table">
              <tbody>
                <tr className="border-b border-gray-100">
                  <td className="font-bold text-sm text-gray-400 uppercase tracking-wider w-1/3 py-4">{t('products:detail.general.description')}</td>
                  <td className="uppercase font-black text-sm py-4">{selectedProduit.description || '-'}</td>
                </tr>
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.rayon')}</td>
                  <td className="py-4"><span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-100">{selectedProduit.rayon_name || '-'}</span></td>
                </tr>
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.provider')}</td>
                  <td className="py-4"><span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200">{selectedProduit.fournisseur_name || '-'}</span></td>
                </tr>
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.min_max')}</td>
                  <td className="font-black py-4">{selectedProduit.stock_minimum ?? 0} / {selectedProduit.stock_maximum ?? 0}</td>
                </tr>
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.alert_threshold')}</td>
                  <td className="py-4"><span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-100">{selectedProduit.stock_alert ?? 0}</span></td>
                </tr>
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.expiration')}</td>
                ...
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.last_purchase')}</td>
                  <td className="font-mono font-bold text-sm py-4">{formatDate(selectedProduit.dernier_achat)}</td>
                </tr>
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.last_sale')}</td>
                  <td className="font-mono font-bold text-sm py-4">{formatDate(selectedProduit.dernier_vente)}</td>
                </tr>
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.lot_management')}</td>
                ...
                <tr className="border-b border-gray-100">
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.prescription')}</td>
                ...
                <tr>
                   <td className="font-bold text-sm text-gray-400 uppercase tracking-wider py-4">{t('products:detail.general.surveillance')}</td>
        ...
        {activeTab === 'prix' && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="bg-gray-50 rounded-xl border border-gray-100 p-5">
               <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{t('products:detail.price.cost')}</div>
            ...
            <div className="bg-gray-50 rounded-xl border border-gray-100 p-5">
               <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{t('products:detail.price.vat')}</div>
              <div className="text-2xl font-bold text-gray-900">{selectedProduit.tva || '19.25'}%</div>
            </div>
            <div className="bg-gray-50 rounded-xl border border-gray-100 p-5">
               <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{t('products:detail.price.margin_percent')}</div>
              <div className="text-2xl font-bold text-emerald-600">{Number(selectedProduit.pourcentage_marge || 0).toFixed(1)}%</div>
            </div>
            <div className="bg-gray-50 rounded-xl border border-gray-100 p-5">
               <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{t('products:detail.price.margin_coeff')}</div>
              <div className="text-2xl font-bold text-emerald-600">{Number(selectedProduit.taux_marge || 0).toFixed(2)}</div>
            </div>
            <div className="bg-gray-50 rounded-xl border border-gray-100 p-5">
               <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{t('products:detail.price.rotation')}</div>
               <div className="text-2xl font-bold text-blue-600">{Number(selectedProduit.rotation_moyenne || 0).toFixed(1)}<span className="text-xs font-bold uppercase ml-1 text-gray-400"> {t('products:detail.price.per_month')}</span></div>
            </div>
          </div>
        )}
