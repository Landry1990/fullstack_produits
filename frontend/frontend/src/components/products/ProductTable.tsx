    ...
    <div className="flex-1 overflow-x-hidden overflow-y-auto">
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center h-full">
          ...
          <h3 className="text-base font-semibold text-gray-500">{t('products:table.empty_title', { defaultValue: 'Aucun produit' })}</h3>
          <p className="text-gray-400 text-sm mt-1 max-w-sm">{t('products:table.empty_subtitle', { defaultValue: 'Créez votre premier produit ou service pour commencer.' })}</p>
          <button
            className="inline-flex items-center gap-2 px-6 py-2 text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 mt-6 transition-colors"
            onClick={() => {
              const parentBtn = document.querySelector('button.bg-indigo-600');
        ...
        <>
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-gray-50 cursor-pointer transition-colors ${
                        ...
                        : isChecked
                          ...
                          : 'text-gray-700'
                      } ${stock < 0 ? 'text-red-600' : stock === 0 ? 'opacity-60 text-gray-400' : ''}`}
                    ...
                    >
                      ...
                      <td className="py-3 px-3 w-32">
                        <span className="font-mono text-xs text-gray-400">{produit.cip1 || '-'}</span>
                      </td>
                      <td className="py-3 px-3">
                        <div
                          className={`text-sm flex items-center flex-wrap gap-2 ${
                            stock < 0 ? 'text-red-600 font-semibold' :
                            stock === 0 ? 'text-gray-400 font-medium' :
                        ...
                        >
                          ...
                          {produit.is_supplier_exclusive && (
                            <span
                              className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-50 text-emerald-700 border border-emerald-100 shrink-0"
          ...
          <div className="md:hidden flex flex-col w-full px-2 py-2 gap-2 bg-gray-50">
            {products.map(produit => {
              const stock = produit.stock ?? 0;
              const isSelected = selectedProduit?.id === produit.id;
              const isChecked = selectedProductIds.includes(produit.id);
              ...
              return (
                <div
                  key={produit.id}
                  data-product-id={produit.id}
                  className={`flex flex-col p-3 rounded-lg border bg-white shadow-sm transition-all relative overflow-hidden ${
                    isSelected ? 'border-indigo-300 shadow-sm' : isChecked ? 'border-emerald-300' : 'border-gray-100 hover:border-gray-200'
                  }`}
                  onClick={() => onViewDetails(produit)}
                >
                  ...
                  <div className="flex justify-between items-start w-full gap-3 pl-1">
                    ...
                    <div className="flex-1 min-w-0 pr-1 py-0.5">
                       <div className="flex items-center justify-between w-full mb-0.5">
                          <div className={`text-sm font-semibold truncate tracking-tight uppercase ${stock < 0 ? 'text-red-600' : stock === 0 ? 'text-gray-400' : 'text-gray-900'}`}>
                             {produit.name}
                          </div>
                       </div>
                       ...
                       <div className="flex items-center gap-2 mb-2">
                           {produit.is_supplier_exclusive && (
                               <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-50 text-emerald-700 border border-emerald-100">EXCLUSIF</span>
                           )}
                           <span className="text-xs text-gray-400 font-mono font-medium tracking-tight">
                              {produit.cip1 || 'AUCUN CIP'}
                           </span>
                       </div>

                        <div className="flex items-center justify-between w-full pt-1.5 border-t border-gray-100">
                          <div className="flex items-center gap-1">
                             <span className="text-[10px] text-gray-400 font-medium uppercase">Stock</span>
                             <span className={`text-xs font-semibold ${stock < 0 ? 'text-red-500' : stock === 0 ? 'text-gray-400' : 'text-gray-700'}`}>
                                {stock}
                             </span>
                          </div>
