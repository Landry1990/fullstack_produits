...
export default function EtatsInventaire() {
  ...
  const groupByOptions = [
    ...
    { value: 'FORME', label: 'Par Forme' },
    { value: 'GROUPE', label: 'Par Groupe' },
  ];

  const stockOptions = [
    { value: 'MACHINE', label: 'Stock Machine (valeurs actuelles)' },
    { value: 'ZERO', label: 'Stock à Zéro (pour saisie)' },
  ];

  const getEntityLabel = () => {
    return groupBy === 'RAYON' ? 'Rayon' : groupBy === 'FORME' ? 'Forme' : 'Groupe';
  };

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">États d'Inventaires</h1>
      </div>

      <div className="card bg-base-100 shadow-xl">
        <div className="card-body">
          <h2 className="card-title text-lg mb-4">Générer un état d'inventaire</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Regroupement */}
            <div className="form-control">
              <label className="label">
                <span className="label-text font-semibold">Regroupement</span>
              </label>
            ...
            <div className="form-control">
              <label className="label">
                <span className="label-text font-semibold">{getEntityLabel()} à imprimer</span>
              </label>
              <select
              ...
              >
                <option value="">Tous les {getEntityLabel().toLowerCase()}s</option>
                {entities.map(entity => (
                  <option key={entity.id} value={entity.id}>{entity.name}</option>
                ))}
              </select>
              {loadingEntities && <span className="text-xs text-base-content/60 mt-1">Chargement...</span>}
            </div>

            {/* Affichage Stock */}
            <div className="form-control">
              <label className="label">
                <span className="label-text font-semibold">Affichage du Stock</span>
              </label>
              <div className="flex flex-col gap-2">
          ...
          <div className="bg-base-200 rounded-lg p-4">
            <p className="text-sm">
              <strong>Récapitulatif :</strong> État d'inventaire 
              ...
              {selectedEntity ? (
                ...
                <span className="badge badge-ghost badge-sm mx-1">Tous</span>
              )}
              avec
      ...
      <div className="alert alert-info">
        ...
        <div>
          <h3 className="font-bold">Comment utiliser ?</h3>
          <div className="text-xs">
            1. Choisissez le type de regroupement<br/>
            2. Sélectionnez un {getEntityLabel().toLowerCase()} spécifique ou laissez "Tous"<br/>
            3. Choisissez l'affichage du stock<br/>
