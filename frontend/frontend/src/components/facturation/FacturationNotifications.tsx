        ...
        <div role="alert" className="flex items-start gap-3 p-4 rounded-lg bg-[#dcfce7] text-[#14532d] dark:bg-emerald-900/20 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 shadow-lg max-w-lg flex-col items-start gap-2 animate-in fade-in slide-in-from-right-5 duration-300">
          ...
          <div className="flex flex-wrap gap-2 w-full justify-end mt-1">
            ...
            {successInfo.status !== 'PAY' && (
              <Button variant="default" size="sm" onClick={() => onOpenPaymentModal(successInfo)}>
                ...
                {t('facturation:common.pay')}
              </Button>
            )}
            
            {/* Bouton Ticket */}
            {successInfo.status === 'PAY' && Boolean(ticketCaisse) && (
              <Button variant="default" size="sm" className="bg-sky-500 hover:bg-sky-600 text-white" onClick={() => onShowTicket()}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4 mr-1">
                ...
                {t('facturation:common.receipt')}
              </Button>
            )}
            
            {/* Bouton Impression A4 */}
            <Button variant="outline" size="sm" onClick={() => onPrintA4(successInfo)}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4 mr-1">
              ...
              {t('facturation:actions.format_a4')}
            </Button>
          </div>
        </div>
      )}
