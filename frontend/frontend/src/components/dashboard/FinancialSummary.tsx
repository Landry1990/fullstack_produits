...
export default function FinancialSummary({
  ...
  return (
    <div className="space-y-6">
      ...
      {showUG && (
        <div className="bg-base-100 rounded-xl shadow-sm border border-base-200 rounded-xl overflow-hidden">
          <div className="p-6">
          ...
          <div className="p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr>
                    <th className="bg-base-200 text-xs font-semibold uppercase tracking-wider text-base-content/60 py-3 pl-4 rounded-l-2xl text-left">{t('debts.supplier')}</th>
                    <th className="bg-base-200 text-xs font-semibold uppercase tracking-wider text-base-content/60 py-3 text-left">{t('debts.invoice')}</th>
                    <th className="bg-base-200 text-xs font-semibold uppercase tracking-wider text-base-content/60 py-3 text-right">{t('debts.amount_due')}</th>
                    <th className="bg-base-200 text-xs font-semibold uppercase tracking-wider text-base-content/60 py-3 text-center">{t('debts.due_date')}</th>
                    <th className="bg-base-200 text-xs font-semibold uppercase tracking-wider text-base-content/60 py-3 pr-4 rounded-r-2xl text-center">{t('debts.status')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-base-200">
                  {echeances.slice(0, 10).map((e, _i) => {
                    ...
                    return (
                      <tr key={e.numero_facture} className={`transition-all group ${
                        ...
                        <td className="py-3 text-center text-xs font-bold text-base-content/60">
      ...
      <div className="bg-base-100 rounded-xl shadow-sm border border-base-200 rounded-xl overflow-hidden w-full">
          ...
          <div className="flex-grow h-[300px]">
            {stats?.payment_mix && stats.payment_mix.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={stats.payment_mix.map((item) => ({ ...item, value: Math.abs(item.value) }))}
                    cx="50%"
                    cy="50%"
                    innerRadius={55}
                    outerRadius={75}
                    ...
                    nameKey="label"
                  >
                    {stats.payment_mix.map((entry, index: number) => (
          ...
          {stats?.payment_mix && stats.payment_mix.length > 0 && (
            <div className="mt-4 space-y-2 border-t border-base-200 pt-4 shrink-0">
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                {stats.payment_mix.map((item, index: number) => (
