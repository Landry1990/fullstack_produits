    ...
    return (
        <div className="bg-base-100 rounded-xl shadow-sm border border-base-300 overflow-hidden flex flex-col">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                {reports.map((report) => (
                    <div key={report.id} className="p-6 rounded-xl bg-base-200 border border-base-300 hover:bg-base-100 hover:border-primary/20 transition-all flex flex-col h-full group">
                        <div className={`p-4 rounded-xl bg-base-100 shadow-sm w-fit mb-4 text-${report.color}`}>
                            {report.icon}
                        </div>
                        <h4 className={`font-bold text-sm uppercase tracking-tight text-${report.color} mb-2`}>
                            {report.title}
                        </h4>
                        <p className="text-xs text-base-content/60 mb-6 flex-1">
                            {report.desc}
                        </p>
                        <button 
