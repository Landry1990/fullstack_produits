...
interface StockIntelligenceProps {
  ...
  promisDisponibles: PromisDisponible[];
...
export default function StockIntelligence({
  ...
  useEffect(() => {
    api.get('stock-analysis/unsold/', {
      ...
      const items = (res.data?.items ?? []) as DormantItem[];
  ...
  useEffect(() => {
    api.get('stock-analysis/overstock/', {
      ...
      const items = (res.data?.items ?? []) as OverstockItem[];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 auto-rows-fr">
              ...
              {expiringLots.length === 0 ? (
                ...
                expiringLots.slice(0, 10).map((lot) => {
                  ...
                  if (daysUntilExpiry <= 7) {
                    ...
                    urgencyStyle = 'bg-amber-50 border-orange-200 text-orange-900';
        ...
        {reapproStats && reapproStats.product_count > 0 && (
          <div className="bg-cyan-50 rounded-xl shadow-sm border border-blue-100 overflow-hidden h-full flex flex-col">
            <div className="p-6 flex flex-col h-full">
              <div
                className="flex items-center justify-between mb-4 cursor-pointer hover:opacity-80 transition-opacity shrink-0"
