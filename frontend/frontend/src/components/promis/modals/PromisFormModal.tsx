            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-1.5">
                    ...
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            className="w-full rounded-lg border border-gray-200 bg-white pl-9 text-sm font-medium text-gray-900 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-50 transition-all"
                        ...
                        {clientSearch && !formData.client && (
                            <button
                                type="button"
                                className="absolute right-3 top-1/2 -translate-y-1/2 p-1 bg-gray-100 rounded-full hover:bg-gray-200"
                                onClick={() => {
                                    setClientSearch('');
                    ...
                    {clientSearch && !formData.client && clients.filter(c => 
                        ...
                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-100 rounded-xl shadow-md max-h-48 overflow-y-auto">
                            {clients.filter(c => 
                                c.name.toLowerCase().includes(clientSearch.toLowerCase())
                            ).slice(0, 5).map(c => (
                                <div 
                                ...
                                >
                                    ...
                                    <div className="text-xs text-gray-500 font-mono mt-0.5">{c.phone || t('common:no_number', 'Sans numéro')}</div>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="space-y-1.5">
                    ...
                    <input
                        type="text"
                        className="w-full rounded-lg border border-gray-200 bg-white px-3 text-sm font-mono text-gray-900 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-50 transition-all"
                    ...
                    {productSearch && !formData.produit && produits.filter(p => 
                        ...
                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-100 rounded-xl shadow-md max-h-48 overflow-y-auto">
                            {produits.filter(p => 
                                ...
                                <div 
                                    key={p.id} 
                                    className="p-3 hover:bg-indigo-50 cursor-pointer border-b border-gray-100 last:border-0 flex justify-between items-center transition-colors"
                                    onClick={() => selectProduct(p)}
                                >
                                    <div>
                                        <div className="font-medium text-sm">{p.name}</div>
                                        {p.cip1 && <div className="text-xs text-gray-500 font-mono mt-0.5">{p.cip1}</div>}
                                    </div>
                ...
                <div className="space-y-1.5">
                    ...
                    <textarea
                        className="w-full rounded-lg border border-gray-200 bg-white p-3 text-sm font-medium text-gray-900 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-50 transition-all h-24"
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 mt-6">
                    ...
                    <button 
                        type="submit" 
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-gray-600 hover:bg-gray-50 rounded-lg text-sm font-medium transition-colors btn-primary text-white shadow-sm hover:shadow-md transition-all gap-2 px-6"
                        disabled={saving}
                    >
                        {saving && <span className="inline-block size-4 border-2 border-gray-200 border-t-indigo-600 rounded-full animate-spin" />}
                        {t('common:save')}
                    </button>
                </div>
            </form>
