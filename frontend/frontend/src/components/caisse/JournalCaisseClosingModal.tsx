    ...
    <dialog open className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm w-full h-full p-0 m-0 border-none">
      <div className="w-full max-w-lg p-0 overflow-hidden rounded-2xl border border-slate-200 shadow-2xl max-h-[90vh] flex flex-col bg-white">
            ...
            <>
              ...
              {(closingTotals.total_ca_pharmacie !== undefined || closingTotals.total_ca_divers !== undefined) && (
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                  <div className="px-4 py-2 bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
                    {t('closing.sales_breakdown')}
                  </div>
                  <div className="p-3 space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-500 flex items-center gap-1.5">
                        <span className="inline-block size-2 rounded-full bg-emerald-500" />
                        {t('closing.pharmacy_sales')}
                      </span>
                    ...
                    <div className="flex justify-between text-xs pt-1 mt-1 border-t border-dashed border-slate-200">
                      <span className="font-bold text-slate-600">{t('closing.total_sales')}</span>
              ...
              {fondDeCaisse > 0 && (
                <div className="flex justify-between items-center p-3 bg-sky-50 border border-sky-200 rounded-xl">
                  <span className="text-xs font-bold text-sky-600 uppercase">{t('closing.initial_fund')}</span>
                ...
                <div className="flex gap-2 text-xs">
                  {closingTotals.total_entrees > 0 && (
                    <div className="flex-1 p-2 bg-emerald-50/50 border border-emerald-200 rounded-lg text-center">
                      <div className="text-emerald-600/60 uppercase font-bold">{t('closing.db_entries')}</div>
                  ...
                  {closingTotals.total_sorties > 0 && (
                    <div className="flex-1 p-2 bg-red-50/50 border border-red-200 rounded-lg text-center">
                      <div className="text-red-600/60 uppercase font-bold">{t('closing.db_exits')}</div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-3 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <option value="SORTIE">{t('closing.exit')}</option>
                    <option value="ENTREE">{t('closing.entry')}</option>
                  </select>
                  <input
                    type="text"
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <Plus className="size-4 mr-1" /> {t('closing.add')}
                  </Button>
                </div>
              </div>
              ...
              {manualMovements.length > 0 && (
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                  <div className="px-4 py-2 bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
                    {t('closing.manual_movements', { count: manualMovements.length })}
                  </div>
        ...
        <div className="p-5 border-t border-slate-200 bg-white shrink-0 space-y-3">
          {detectedShift?.hasActiveSession && (
            <div className="p-3 bg-red-50 border border-red-200 rounded-xl text-center">
              <p className="text-xs font-bold text-red-600">
                {t('closing.active_session_blocked', { defaultValue: 'Ce caissier a une session active. Fermez d\'abord la caisse avant de clôturer.' })}
              </p>
            </div>
          )}
          <Button
            ...
            disabled={loading || !actualAmount || detectedShift?.hasActiveSession}
          >
