...
export const TrancheHoraireStats: React.FC<TrancheHoraireStatsProps> = ({ onVerify, startDate, endDate }) => {
    ...
    const fetchTrancheStats = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.detail || t('tranche_horaire.error_loading'));
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-base-content">
                            {t('tranche_horaire.title')}
                        </h3>
                        <p className="text-xs text-base-content/60">
                            {t('tranche_horaire.subtitle')}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-end gap-4">
                <div className="form-control">
                    <label className="label py-1">
                        <span className="label-text text-xs font-semibold">{t('tranche_horaire.start_time')}</span>
                    </label>
                    <input 
                        type="time" 
                ...
                <div className="form-control">
                    <label className="label py-1">
                        <span className="label-text text-xs font-semibold">{t('tranche_horaire.end_time')}</span>
                    </label>
                    <input 
                        type="time" 
                        className="input input-bordered input-sm"
