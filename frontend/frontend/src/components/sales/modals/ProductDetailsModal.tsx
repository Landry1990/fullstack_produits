...
export const ProductDetailsModal: React.FC<ProductDetailsModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 p-4">
            <div className="bg-base-100 rounded-xl shadow-2xl border border-base-200 w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
                {/* Header */}
                <div className="border-b border-base-200 p-6 flex justify-between items-center bg-base-200/50">
                    <div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-base-content/60">
                            <span className="flex items-center gap-1.5 shrink-0">
                                <span className="font-mono font-medium text-base-content">#{facture.numero_facture || facture.id}</span>
                            </span>
                            ...
                            <span className="flex items-center gap-1.5 min-w-0">
                                ...
                                <span className="truncate max-w-[200px] sm:max-w-none font-medium text-base-content">
                            ...
                            <span className="flex items-center gap-1.5 shrink-0" title={t('table.operator')}>
                                ...
                                <span className="text-base-content/50 text-xs">{t('table.operator')} :</span>
                                <span className="text-base-content/70 font-medium">{facture.created_by_name || '-'}</span>
                            </span>
                            {facture.paiements && facture.paiements.length > 0 && facture.paiements[0].user_details ? (
                                <>
                                    ...
                                    <span className="flex items-center gap-1.5 shrink-0" title="Caissier">
                                        ...
                                        <span className="text-base-content/50 text-xs">Caissier :</span>
                                        <span className="text-base-content/70 font-medium">{facture.paiements[0].user_details.full_name || facture.paiements[0].user_details.username}</span>
                                    </span>
                                </>
                                ...
                                <>
                                    ...
                                    <span className="flex items-center gap-1.5 shrink-0" title="Caissier">
                                        ...
                                        <span className="text-base-content/50 text-xs">Caissier :</span>
                                        <span className="text-base-content/70 font-medium">{facture.validated_by_name}</span>
                                    </span>
                                </>
                            )}
                            {facture.cancelled_by_name && (
                                <>
                                    ...
                                    <span className="flex items-center gap-1.5 shrink-0" title="Annulé par">
                                        <div className="size-3.5 rounded-full bg-error/20 flex items-center justify-center">
                                            <User className="size-2.5 text-error" />
                                        </div>
                                        <span className="text-base-content/70 font-medium">{facture.cancelled_by_name}</span>
                                    </span>
                                </>
                            )}
                        </div>
                            ...
                            <div className="mt-2 flex items-center gap-2 text-xs text-base-content/60">
                                ...
                                <div className="flex gap-2">
                                    {facture.paiements.map((p, idx) => (
                                        <span key={idx} className="bg-base-200 px-1.5 py-0.5 rounded text-base-content border border-base-300 text-[10px]">
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 hover:bg-base-200 rounded-lg transition-colors text-base-content/50 hover:text-base-content/70"
                    >
                        <X className="size-6" />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-auto p-0">
                    {loading ? (
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-base-200 sticky top-0 z-10 text-xs uppercase text-base-content/60 font-semibold tracking-wider">
                                <tr>
                                    <th className="px-6 py-4 border-b border-base-200">{t('fields.product')}</th>
                                    <th className="px-6 py-4 text-center border-b border-base-200">{t('fields.quantity')}</th>
                                    <th className="px-6 py-4 text-right border-b border-base-200">{t('fields.unit_price')} (Net)</th>
                                    <th className="px-6 py-4 text-right border-b border-base-200">{t('fields.total')}</th>
                                    <th className="px-6 py-4 text-center border-b border-base-200">{t('table.actions')}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50 text-sm">
                                {facture.produits.map((prod) => {
                                    ...
                                    return (
                                        <tr key={prod.id} className="hover:bg-base-200/50 transition-colors group">
                                            <td className="px-6 py-3">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {prod.is_chronic && (
                                                        <span className="flex items-center gap-1 text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-full border border-indigo-100 font-bold animate-pulse">
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-1">
                                                    {prod.lot && (
                                                        <span className="text-[10px] text-info font-mono bg-info/10 px-1.5 py-0.5 rounded border border-blue-100">
                                                            Lot: {prod.lot} {prod.date_expiration && `(Exp: ${prod.date_expiration})`}
                                                        </span>
                                                    )}
                                                    {remiseUnitaire > 0 && (
                                                        <span className="text-[10px] text-warning font-medium bg-warning/10 px-1.5 py-0.5 rounded border border-orange-100">
                                            ...
                                            <td className="px-6 py-3 text-center">
                                                <span className="font-mono bg-base-200 px-2 py-1 rounded text-base-content font-medium">
                                                    {prod.quantity}
                                                </span>
                                            </td>
                                             <td className="px-6 py-3 text-right text-base-content/70 font-mono">
                                                {formatCurrency(puNet)}
                                            </td>
                                            <td className="px-6 py-3 text-right font-medium font-mono text-base-content group-hover:text-primary transition-colors">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all ${
                                                            sendingReminder === prod.id
                                                                ...
                                                                : 'border border-emerald-200 text-success bg-success/10 hover:bg-success/20 hover:shadow-sm'
                                                        }`}
                ...
                {/* Footer Totals */}
                <div className="bg-base-200 border-t border-base-200 p-6">
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-8 text-sm">
                        <div className="flex flex-col items-end border-r border-base-300 pr-8 last:border-0 last:pr-0">
                            <span className="text-[10px] uppercase font-black tracking-widest text-base-content/50 mb-1">{t('fields.subtotal_ht')}</span>
                            <span className="font-mono font-bold text-base-content">{formatCurrency(totals.totalHt)}</span>
                        </div>
                        <div className="flex flex-col items-end border-r border-base-300 pr-8 last:border-0 last:pr-0">
                            <span className="text-[10px] uppercase font-black tracking-widest text-base-content/50 mb-1">{t('fields.vat')}</span>
                        ...
                        {totals.remise > 0 && (
                            <div className="flex flex-col items-end border-r border-base-300 pr-8 last:border-0 last:pr-0">
                                ...
                                <span className="font-mono font-bold text-warning">-{formatCurrency(totals.remise)}</span>
                            </div>
                        )}
                        <div className="flex flex-col items-end bg-info/10 px-4 py-2 rounded-xl border border-blue-100 shadow-sm">
                            <span className="text-[10px] uppercase font-black tracking-widest text-blue-500 mb-1">{t('fields.total_ttc')}</span>
                            <span className="font-mono text-xl text-info font-black tracking-tight">
                    ...
                    <div className="flex justify-end mt-4">
                        {Math.abs(totals.partClient - totals.totalTtc) > 1 && (
                            <div className="flex items-center gap-4 text-white bg-info py-2.5 px-5 rounded-xl shadow-lg animate-in slide-in-from-bottom-1 duration-500">
