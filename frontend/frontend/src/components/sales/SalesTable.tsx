...
const BulkActionsMenu: React.FC<BulkActionsMenuProps> = React.memo(({
    ...
    if (selectedIds.length === 1) {
        ...
        return (
            <>
                <li className="text-[10px] font-medium text-base-content/50 px-4 py-2 uppercase tracking-widest">{t('common:single_selection', { defaultValue: 'Sélection' })}</li>
                <li><a onClick={() => onView(selectedFacture)} className="gap-3 py-3"><Eye className="size-4 text-base-content/60" />{t('common:details')}</a></li>
    ...
    return (
        <>
            <li className="text-[10px] font-medium text-base-content/50 px-4 py-2 uppercase tracking-widest">{t('common:bulk_actions')}</li>
...
export const SalesTable: React.FC<SalesTableProps> = ({
    ...
    const getStatusStyle = (status: string) => {
        const normalized = normalizeStatus(status);
        switch (normalized) {
            case 'PAY': return 'bg-success/10 text-success border-emerald-100';
            case 'ANN': return 'bg-error/10 text-error border-red-100';
            case 'BROU': return 'bg-base-200 text-base-content/60 border-base-300';
            default: return 'bg-warning/10 text-warning border-amber-100';
        }
    };
    ...
    if (loading)         return (
             <div className="flex flex-col items-center justify-center py-20 text-base-content/50">
                <div className="size-10 border-2 border-base-300 border-t-indigo-600 rounded-full animate-spin mb-3"></div>
                <p className="text-sm font-medium text-base-content/60">{t('common:loading')}</p>
            </div>
         );

    if (factures.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-base-content/50 bg-base-200/50 m-4 rounded-2xl border border-dashed border-base-300">
                <h3 className="text-base font-semibold text-base-content mb-1">{t('sales:no_sales_found')}</h3>
                <p className="text-sm text-base-content/60">{t('sales:try_different_filters')}</p>
            </div>
        );
    }
            ...
            <table className="w-full border-separate border-spacing-0">
                    ...
                    <tr className="bg-base-200 text-left text-xs font-semibold text-base-content/60 uppercase tracking-wider">
                        <th className="px-4 py-4 w-10 sticky top-0 bg-base-200 border-b border-base-300">
                            <input
                                ...
                                className="size-3.5 rounded border-base-300 text-primary focus:ring-primary"
                ...
                <tbody className="divide-y divide-base-200">
                    {factures.map((facture) => (
                        <tr 
                            key={facture.id}
                            className={`group transition-colors duration-150 ${selectedIds.includes(facture.id) ? 'bg-primary/10/30 hover:bg-primary/10/50' : 'hover:bg-base-200'}`}
                        >
                            ...
                            <td className="px-6 py-4 text-center">
                                <span className="inline-flex items-center px-1.5 py-1 rounded-lg bg-base-200 text-base-content font-mono font-bold text-sm border border-base-300 group-hover:bg-base-100 group-hover:border-indigo-200 group-hover:text-primary group-hover:shadow-sm transition-all whitespace-nowrap">
                            ...
                            <td className="px-6 py-4 text-center whitespace-nowrap">
                                <span className={`${normalizeNumberInput(facture.montant_en_compte || '0') > 0 ? 'text-warning' : 'text-base-content/40'} font-bold font-mono text-sm uppercase`}>
                            ...
                            <td className="px-6 py-4 text-center hidden md:table-cell">
                                <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${getStatusStyle(facture.status)}`}>
                            ...
                            <td className="px-6 py-4 text-right">
                                {selectedIds.length === 0 && (
                                    <div className="flex justify-end gap-1 text-base-content/20 group-hover:opacity-100 transition-opacity">
                                        <ActionIcon 
                                            icon={Eye}
