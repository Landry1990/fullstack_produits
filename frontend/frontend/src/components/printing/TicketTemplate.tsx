    ...
    >
          ...
          <table className="w-full text-[9px] font-mono">
            <tbody>
              ...
              <tr>
                ...
                <td className="pt-1 text-right text-[10px] font-medium uppercase">{clientName}</td>
              </tr>
              {ticket.client_solde_depot && Number(ticket.client_solde_depot) > 0 && (
                <tr className="mt-1 border-t border-black/20">
                  ...
                  <td className="py-1 text-right text-[10px] font-medium">{formatM(ticket.client_solde_depot)}</td>
                </tr>
              )}
              {ticket.client_points_fidelite !== undefined && ticket.client_points_fidelite !== null && (
                <tr className={ticket.client_solde_depot && Number(ticket.client_solde_depot) > 0 ? "" : "mt-1 border-t border-black/20"}>
                  ...
                  <td className="py-1 text-right text-[10px] font-medium">{ticket.client_points_fidelite} {t('ticket.pts')}</td>
                </tr>
              )}
              <tr>
      ...
      <div className="mb-2">
          <table className="w-full border-collapse" style={{ tableLayout: 'fixed' }}>
            <thead>
              <tr className="border-b border-black/30 text-[9px] font-semibold uppercase">
      ...
      <div className="mt-2 space-y-1">
        ...
        <div className="flex items-center justify-between border-y border-black/30 py-1.5 font-semibold">
            <span className="text-[10px] uppercase tracking-tight">{t('ticket.net_a_payer_cfa')}</span>
        ...
        {(ticket.total_lettres || facture?.total_lettres) && (
            <div className="border-b border-black/20 py-1.5 text-center text-[9px] font-medium italic uppercase">
                {ticket.total_lettres || facture?.total_lettres}
            </div>
        )}
        ...
        {showBarcode && (
            <div className="inline-block px-2 bg-base-100 text-center">
                <Barcode 
                    ...
                    background="#ffffff"
                />
                <div className="mt-1 font-mono text-[10px] font-medium">{barcodeValue}</div>
            </div>
        )}
