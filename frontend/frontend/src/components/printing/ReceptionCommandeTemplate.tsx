...
export const ReceptionCommandeTemplate: React.FC<ReceptionCommandeTemplateProps> = ({ commande, settings, fournisseurName }) => {
    ...
    return (
        <div className="p-4 bg-white text-black font-sans text-[11.5px] leading-tight max-w-[210mm] mx-auto shadow-none print:shadow-none print:p-0">
            
            {/* EN-TETE PHARMACIE */}
            <div className="flex justify-between items-start mb-4 border-b-2 border-slate-900 pb-3">
                <div className="flex-1">
                    <h1 className="text-xl font-black uppercase tracking-tight text-slate-900 mb-0.5">
                        {settings.pharmacy_name || 'PHARMACIE'}
                    </h1>
                    <div className="text-slate-500 space-y-0.5 text-[10.5px]">
                ...
                <div className="text-right">
                    <div className="border-2 border-slate-900 text-slate-900 px-3 py-1.5 rounded text-base font-bold mb-1 inline-block uppercase">
                        {t('orders.tabs.delivery', { defaultValue: 'BON DE RÉCEPTION' })}
                    </div>
                    <div className="text-slate-500 font-bold text-[9px] uppercase tracking-widest">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-white p-3 rounded-lg border border-slate-200">
                    <div className="text-[9px] uppercase tracking-widest font-black text-slate-400 mb-1.5 border-b border-slate-100 pb-1">
                        {t('orders.form.provider_label', { defaultValue: 'Fournisseur' })}
                    </div>
                    <div className="font-black text-base text-slate-900">{fournisseurName.toUpperCase()}</div>
                </div>
                <div className="bg-white p-3 rounded-lg border border-slate-200">
                    <div className="text-[9px] uppercase tracking-widest font-black text-slate-400 mb-1.5 border-b border-slate-100 pb-1">
                    ...
                    <div className="space-y-0.5 text-[11px]">
                        <div className="flex justify-between">
                            <span className="text-slate-500">{t('orders.details.date', { defaultValue: 'Date Commande' })}:</span>
                            <span className="font-bold">{formatDate(commande.date)}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-slate-500">{t('common:print_date', { defaultValue: 'Imprimé le' })}:</span>
                            <span className="font-bold">{now}</span>
                        </div>
                        <div className="flex justify-between border-t border-slate-100 pt-1 mt-1">
                            <span className="text-slate-500">{t('orders.details.created_by', { defaultValue: 'Saisie par' })}:</span>
                            <span className="font-bold">{(commande as any).created_by_name || 'N/A'}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-slate-500">{t('orders.details.closed_by', { defaultValue: 'Clôturée par' })}:</span>
            ...
            <div className="mb-4">
                <table className="w-full border-collapse text-[10.5px]">
                    <thead className="table-header-group">
                        <tr className="bg-slate-50 text-slate-900 border-b-2 border-slate-900 text-[8.5px] uppercase tracking-wider">
                    ...
                    <tbody>
                        {produits.map((p, idx) => {
                            ...
                            const tvaLabel = p.tva ? <span className="text-[7.5px] text-slate-400 ml-1">({p.tva}%)</span> : null;
                            ...
                            return (
                                <tr key={idx} className="border-b border-slate-100 break-inside-avoid">
                                    <td className="py-1 px-3">
                                        <div className="font-bold text-slate-800 uppercase text-[10px] leading-tight">{produitName}</div>
                                        {p.lot && <div className="text-[7.5px] text-slate-500 font-mono">LOT: {p.lot} | EXP: {p.date_expiration ? new Date(p.date_expiration).toLocaleDateString() : '-'}</div>}
                                    </td>
                                    <td className="py-1 px-2 text-center font-mono text-[8.5px] text-slate-500">{cip}</td>
                                    <td className="py-1 px-2 text-center font-black">{stAnt}</td>
                                    <td className="py-1 px-2 text-center font-black">{p.quantity}</td>
                                    <td className="py-1 px-2 text-center font-bold text-slate-400 bg-slate-50/20">{p.unites_gratuites || 0}</td>
                                    <td className="py-1 px-2 text-center font-black">{currentStock}</td>
                                    <td className="py-1 px-2 text-right text-slate-600">{formatM(p.price)}{tvaLabel}</td>
                                    <td className="py-1 px-3 text-right font-black text-slate-900">{formatM(lineTotal)}</td>
                                </tr>
                            );
                        })}
                    </tbody>
            ...
            <div className="flex justify-between items-start gap-8 border-t-2 border-slate-900 pt-4">
                <div className="flex-1 space-y-3">
                    <div className="bg-white p-3 rounded-lg border border-slate-200">
                        <div className="text-[9px] uppercase font-black text-slate-400 mb-1.5 tracking-widest">
                            {t('orders.details.financial_summary', { defaultValue: 'Récapitulatif Articles' })}
                        </div>
                        <div className="flex gap-6 text-[10.5px] font-bold">
                            <div>{t('orders.list.table.items', { defaultValue: 'Lignes' })}: <span className="text-slate-900">{produits.length}</span></div>
                            <div>{t('orders.product_table.headers.qty', { defaultValue: 'Unités' })}: <span className="text-slate-900">{produits.reduce((s, p) => s + p.quantity, 0)}</span></div>
                            <div className="text-slate-400">{t('orders.product_table.headers.ug', { defaultValue: 'Gratuites' })}: <span>{totalUG}</span></div>
                        </div>
                        <div className="text-[8.5px] italic text-slate-400 mt-2 leading-tight">
                ...
                <div className="w-64 space-y-3">
                    <div className="border-2 border-slate-900 text-slate-900 rounded-lg p-3">
                        <div className="space-y-0.5 mb-1.5 border-b border-slate-100 pb-1.5">
                           <div className="flex justify-between text-[9px] text-slate-500">
                               ...
                               <span className="font-bold text-slate-900">{formatM(totalHT)} F</span>
                           </div>
                           <div className="flex justify-between text-[9px] text-slate-500">
                               <span>{t('orders.product_table.total_tva', { defaultValue: 'TOTAL TVA' })}:</span>
                               <span className="font-bold text-slate-900">{formatM(totalTVA)} F</span>
                           </div>
                        </div>
                    ...
                    <div className="pt-2 flex flex-col items-center">
                        <div className="text-[8px] uppercase font-black tracking-widest text-slate-400 mb-4 text-center">Cachet & Signature</div>
                        <div className="w-full h-14 border border-slate-200 rounded-lg flex items-center justify-center text-[8px] text-slate-300 uppercase italic">
                            Responsable Stocks
                        </div>
                    </div>
                </div>
            ...
            {/* PIED DE PAGE */}
            <div className="mt-6 pt-3 border-t border-slate-200 text-center text-[9px] font-bold text-slate-300 uppercase tracking-[0.2em]">
