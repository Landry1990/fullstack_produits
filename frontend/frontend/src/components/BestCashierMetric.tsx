    ...
    if (performances.length === 0) {
        return (
            <div className="bg-base-100 rounded-2xl shadow-sm border border-base-300 p-10 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div>
                    <h4 className="font-bold text-base-content/40 uppercase tracking-widest text-xs mb-1">{t('dashboard.no_data_yet', 'Classement en attente')}</h4>
                    <p className="text-sm text-base-content/30 italic max-w-xs mx-auto">
                        {t('dashboard.no_closures_description', 'Aucune clôture de caisse n\'a été enregistrée pour cette période.')}
                    </p>
                </div>
            </div>
        );
            ...
            <div className="lg:col-span-1 bg-primary rounded-2xl shadow-lg p-6 text-primary-content relative overflow-hidden group border border-primary-focus">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="font-black text-lg uppercase tracking-widest">{t('dashboard.best_cashier', 'Meilleur Caissier')}</h3>
                    </div>
                    ...
                    <div className="space-y-4 pt-5 border-t border-white/20">
                        ...
                        <div className="flex justify-between items-center bg-white/10 p-3 rounded-xl border border-white/5">
                            <span className="text-white/80 text-xs font-bold uppercase tracking-wider">{t('dashboard.closures_count', 'Total Clôtures')}</span>
            ...
            <div className="lg:col-span-2 bg-base-100 rounded-2xl shadow-sm border border-base-300 overflow-hidden flex flex-col">
                <div className="px-6 py-5 border-b border-base-200 flex justify-between items-center bg-base-100/50 backdrop-blur-sm">
                    <h3 className="font-black text-base-content flex items-center gap-3 text-lg uppercase tracking-tight">
                        ...
                        {t('dashboard.cashier_ranking', 'Classement de Rigueur')}
                    </h3>
                    ...
                    <table className="table table-zebra w-full border-separate border-spacing-0">
                        <thead>
                            <tr className="bg-base-200/50 border-b border-base-300">
                                <th className="text-[10px] uppercase font-black text-base-content/40 px-6 py-4 tracking-widest">Rang</th>
                                <th className="text-[10px] uppercase font-black text-base-content/40 px-6 py-4 tracking-widest">{t('user_sessions.operator')}</th>
                                <th className="text-[10px] uppercase font-black text-base-content/40 text-center px-6 py-4 tracking-widest">Clôtures</th>
                                <th className="text-[10px] uppercase font-black text-base-content/40 text-right px-6 py-4 tracking-widest">Moyenne Écart</th>
                                <th className="text-[10px] uppercase font-black text-base-content/40 text-right px-6 py-4 tracking-widest">Tendance</th>
                            </tr>
                        </thead>
                            ...
                            {performances.map((perf, index) => (
                                <tr key={perf.user_id} className="hover:bg-primary/5 transition-all duration-200 group">
                                    <td className="px-6 py-4">
                                        {index === 0 ? (
                                            <div className="badge bg-yellow-400 border-none text-yellow-900 font-black px-3 h-7 italic shadow-sm">1er</div>
                                        ) : index === 1 ? (
                                            <div className="badge bg-slate-300 border-none text-slate-700 font-black px-3 h-7 italic shadow-sm">2ème</div>
                                        ) : index === 2 ? (
                                             <div className="badge bg-amber-600 border-none text-amber-50 font-black px-3 h-7 italic shadow-sm">3ème</div>
                                        ) : (
