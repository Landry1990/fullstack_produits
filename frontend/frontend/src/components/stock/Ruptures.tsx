...
export default function Ruptures() {
  ...
  const fetchPharmacieRuptures = async (page = 1) => {
    ...
    try {
      let url = `produits/?stock__lte=0&rotation_moyenne__gte=1&latest_supplier=true&page=${page}&page_size=50`;
    ...
    <div className="p-2 md:p-4 max-w-full mx-auto space-y-4">
      ...
      {activeTab === 'pharmacie' && (
        <div className="space-y-4">
          ...
          <div className="bg-white shadow-sm border border-slate-200 rounded-2xl overflow-hidden flex flex-col h-[65vh]">
            <div className="flex items-center gap-2 p-4 text-emerald-700 bg-emerald-50 border-b border-emerald-100">
              ...
              <div className="overflow-auto flex-1 min-h-0">
                {pharmacieLoading ? (
                  <div className="h-full flex items-center justify-center p-4"><SkeletonTable rows={10} columns={9} /></div>
                ) : pharmacieData.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-300">
                    <ShieldAlert className="size-14 mb-3 opacity-20" />
                    <p className="text-base font-black uppercase tracking-tighter italic">{t('ruptures.pharmacie.empty', 'Aucune rupture critique détectée')}</p>
                  </div>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-slate-50 border-b border-slate-100">
                        ...
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap py-2 text-left">{t('ruptures.pharmacie.columns.cip', 'CIP')}</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap py-2 text-left">{t('ruptures.pharmacie.columns.produit', 'Produit')}</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap py-2 text-left">{t('ruptures.pharmacie.columns.rayon', 'Rayon')}</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap py-2 text-center">{t('ruptures.pharmacie.columns.rotation', 'Rotation')}</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap py-2 text-left">{t('ruptures.pharmacie.columns.fournisseur', 'Fournisseur')}</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap py-2 text-left">{t('ruptures.pharmacie.columns.dernier_achat', 'Dernier Achat')}</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap py-2 text-left">{t('ruptures.pharmacie.columns.prix_achat', 'Prix Achat')}</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-2 text-center">{t('common:actions_title', 'Actions')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {pharmacieData.map((p) => {
                        ...
                        return (
                          <tr key={p.id} className={`hover:bg-slate-50 transition-colors border-b border-slate-100 ${isHighRotation ? 'bg-red-50/40' : ''}`}>
                            ...
                            <td className="font-mono text-xs whitespace-nowrap text-slate-500 py-2">{p.cip1 || '-'}</td>
                            <td className="font-black whitespace-nowrap py-2 text-sm">
                              {p.name}
                            ...
                            <td className="text-sm whitespace-nowrap font-medium text-slate-600 py-2">{p.rayon_name || '-'}</td>
                            ...
                            <td className="text-sm whitespace-nowrap font-medium text-slate-500 py-2">{p.fournisseur_name || p.latest_fournisseur_name || '-'}</td>
                            <td className="text-xs text-slate-500 whitespace-nowrap font-bold py-2">{formatDate(p.dernier_achat)}</td>
                            <td className="font-mono text-xs whitespace-nowrap font-bold text-slate-700 py-2">{formatCurrency(p.cost_price)}</td>
      ...
      {activeTab === 'fournisseur' && (
        <div className="space-y-4">
            <div className="bg-white shadow-sm border border-slate-200 rounded-2xl p-4">
                <h2 className="text-base font-black mb-3 uppercase italic tracking-tighter text-slate-800">{t('ruptures.fournisseur.signaler', 'Signaler un Manque')}</h2>
                <div className="relative max-w-md">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full h-10 pl-10 pr-4 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-sm font-bold text-slate-700 focus:outline-none transition-all"
                            value={searchQuery}
            ...
            <div className="bg-white shadow-sm border border-slate-200 rounded-2xl overflow-hidden flex flex-col h-[65vh]">
                <div className="px-4 py-3 flex items-center justify-between bg-slate-50 border-b border-slate-100">
                    <h2 className="text-base font-black uppercase italic tracking-tighter text-slate-800">
                ...
                <div className="overflow-auto flex-1 min-h-0">
                    {fournisseurLoading ? (
                        <div className="h-full flex items-center justify-center p-4"><SkeletonTable rows={5} columns={5} /></div>
                    ) : fournisseurData.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-300">
                            <ShieldAlert className="size-14 mb-3 opacity-20" />
                            <p className="text-base font-black uppercase tracking-tighter italic">{t('ruptures.fournisseur.empty', 'Aucune rupture fournisseur active')}</p>
                        </div>
                    ) : (
                        <table className="w-full text-sm">
                            <thead className="sticky top-0 z-10">
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    ...
                                    <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-2 text-left">{t('ruptures.fournisseur.columns.produit', 'Produit')}</th>
                                    <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-2 text-left">{t('ruptures.fournisseur.columns.fournisseur', 'Fournisseur')}</th>
                                    <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-2 text-left">{t('ruptures.fournisseur.columns.date', 'Signalé le')}</th>
                                    <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-2 text-right">{t('common:actions_title', 'Actions')}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {fournisseurData.map((r) => (
                                    <tr key={r.id} className="hover:bg-slate-50 transition-colors border-b border-slate-50">
                                        ...
                                        <td className="font-black text-red-500 py-2 text-sm">
                                            {r.produit_nom}
                                            {r.remarques && <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">{r.remarques}</p>}
                                        </td>
                                        <td className="text-sm font-bold text-slate-500 py-2">{r.fournisseur_nom || '-'}</td>
                                        <td className="text-sm font-mono text-slate-400 py-2">
        ...
        <div className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-xs font-medium">{t('ruptures.stats.subtitle', 'Top des produits les plus souvent absents')}</p>
                  </div>
                </div>

                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex items-center gap-0 rounded-xl overflow-hidden border border-slate-200 shadow-sm">
                        <select
                          className="h-9 px-3 bg-slate-50 text-slate-700 text-sm font-bold focus:outline-none border-none"
                          value={statsDays}
                        ...
                        <button
                          onClick={exportStats}
                          className="inline-flex items-center h-9 px-4 bg-slate-800 text-white text-xs font-black gap-2 hover:bg-slate-700 transition-colors"
                        >
           ...
           <div className="bg-white shadow-sm border border-slate-200 rounded-2xl overflow-hidden flex flex-col h-[65vh]">
              <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
                <h3 className="text-xs font-black uppercase tracking-widest text-slate-500">{t('ruptures.stats.table_title', 'Classement des ruptures')}</h3>
              </div>
              <div className="overflow-auto flex-1 min-h-0">
                {statsLoading ? (
                  <div className="h-full flex items-center justify-center p-4"><SkeletonTable rows={10} columns={3} /></div>
                ) : statsData.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-300">
                    <History className="size-14 mb-3 opacity-20" />
                    <p className="text-base font-black uppercase tracking-tighter italic">{t('ruptures.stats.empty', 'Pas de données statistiques pour cette période')}</p>
                  </div>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-slate-50 border-b border-slate-100">
                        <th className="w-20 text-center text-[10px] font-black uppercase tracking-widest text-slate-400 py-2">{t('ruptures.stats.columns.rang', 'RANG')}</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-2 text-left">{t('ruptures.stats.columns.produit', 'PRODUIT')}</th>
                        <th className="text-center text-[10px] font-black uppercase tracking-widest text-slate-400 py-2">{t('ruptures.stats.columns.frequence', 'FRÉQUENCE')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {statsData.map((stat, index) => (
                        <tr key={stat.produit_id} className="hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-b-0 group">
                          <td className="text-center py-2">
                             <div className={`size-7 rounded-lg flex items-center justify-center mx-auto text-xs font-black ${
                               index < 3 ? 'bg-amber-400 text-amber-900 shadow-md' : 'bg-slate-100 text-slate-400'
                             }`}>
                                {index + 1}
                             </div>
                          </td>
                          <td className="font-black group-hover:text-emerald-600 transition-colors text-sm py-2">
                            {stat.produit_name}
                          </td>
                          <td className="text-center py-2">
                              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-black italic bg-slate-100 text-slate-600">
