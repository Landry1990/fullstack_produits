...
export default function ReapproRayon() {
  ...
  const executeTransfer = async (produit: ProduitModel, quantity: number, sudoCreds?: { validatorId: number, password: string }) => {
    ...
    try {
      ...
      toast.success(t('reappro.messages.transfer_success', { count: quantity, name: produit.name }));
      fetchNeedsRefill();
    } catch (error: any) {
  ...
  const handleTransferAll = async () => {
     ...
     if (toProcess.length === 0) {
        toast(t('reappro.messages.no_products_to_refill'), { icon: 'ℹ️' });
        return;
     }
  ...
  const executeAllTransfer = async (sudoCreds?: { validatorId: number, password: string }) => {
     ...
     toast.success(t('reappro.messages.bulk_success', { success, total: toProcessAll.length }));
     fetchNeedsRefill();
     setToProcessAll([]);
  };
  ...
  return (
    <div className="min-h-screen bg-base-200 p-6 space-y-6 font-sans">
      ...
      <div className="bg-base-100 rounded-2xl shadow-sm border border-base-300 overflow-hidden">
        <div className="p-6 border-b border-base-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-base-content tracking-tight flex items-center gap-2">
              <span className="text-primary">📦</span> {t('reappro.title')}
            </h1>
        ...
        <div className="p-4 bg-base-200/30 grid grid-cols-1 md:grid-cols-3 gap-4 no-print">
          ...
          <div className="flex items-center justify-end text-sm text-base-content/60 px-2 font-medium">
            {t('reappro.stats.displayed_products', { count: stats.totalDisplayed })}
          </div>
        </div>
      </div>
      ...
      <div className="print-only mb-8">
        ...
        <p className="mt-2 text-base-content/80">{t('common:date')}: {new Date().toLocaleString(i18n.language === 'fr' ? 'fr-FR' : 'en-GB')}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 no-print">
        <div className="bg-base-100 p-4 rounded-xl shadow-sm border border-base-300 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs uppercase font-bold opacity-50">{t('reappro.stats.urgencies')}</p>
        ...
        <div className="bg-base-100 p-4 rounded-xl shadow-sm border border-base-300 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs uppercase font-bold opacity-50">{t('reappro.stats.suggested_volume')}</p>
            <p className="text-2xl font-black">{stats.totalToTransfer} <span className="text-xs font-normal opacity-70">{t('reappro.stats.units')}</span></p>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="table table-zebra w-full text-base-content">
            <thead className="bg-base-200/50">
              <tr>
                <th className="text-xs uppercase opacity-70">{t('reappro.table.product')}</th>
                <th className="text-xs uppercase opacity-70">{t('reappro.table.level')}</th>
                <th className="text-xs uppercase opacity-70 text-center hidden xl:table-cell">{t('reappro.table.min')}</th>
                <th className="text-xs uppercase opacity-70 text-center hidden xl:table-cell">{t('reappro.table.capacity')}</th>
                <th className="text-xs uppercase opacity-70 text-center">{t('reappro.table.on_shelf')}</th>
                <th className="text-xs uppercase opacity-70 text-center">{t('reappro.table.in_reserve')}</th>
                <th className="text-xs uppercase opacity-70 text-center text-primary">{t('reappro.table.suggestion')}</th>
                <th className="text-xs uppercase opacity-70 text-right no-print">{t('reappro.table.actions')}</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                   <td colSpan={8} className="py-20 text-center">
                      ...
                      <p className="mt-4 text-base-content/60 italic font-medium">{t('reappro.messages.loading_stocks')}</p>
                   </td>
                </tr>
                ...
                <tr>
                   <td colSpan={8} className="py-20 text-center">
                      ...
                      <p className="text-base-content/60 mt-2">{t('reappro.messages.no_reserve_products')}</p>
                   </td>
                </tr>
              ) : (
                    ...
                    <tr key={p.id} className="hover:bg-base-200/30 transition-colors">
                      <td className="max-w-xs">
                        ...
                        <div className="text-[10px] opacity-60 font-mono">
                          {p.rayon_name || t('common:no_rayon', { defaultValue: 'Sans Rayon' })}
                        </div>
                      </td>
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex justify-between items-center px-1">
                             <span className={`text-[10px] font-bold ${isLow ? 'text-error' : 'text-success'}`}>
                                 {isLow ? t('reappro.table.need_refill') : t('reappro.table.stabilized')}
                             </span>
                        ...
                        {suggest > 0 ? (
                            <div className="flex flex-col">
                                <span className="font-black text-primary text-lg">+{suggest}</span>
                                 <span className="text-[10px] text-primary/60 font-bold uppercase">{t('reappro.table.suggested')}</span>
                             </div>
                         ) : (
                             <span className="text-xs opacity-30 italic">{t('reappro.table.complete')}</span>
                         )}
                      </td>
                      <td className="text-right no-print">
                        ...
                        >
                          {!isTransferring && `🚚 ${t('reappro.table.transfer_btn')}`}
                        </button>
                      </td>
                    </tr>
                  );
        ...
        {!loading && filteredProducts.length > itemsPerPage && (
            <div className="p-4 border-t border-base-200 flex items-center justify-between bg-base-50/50 no-print">
                <div className="text-sm text-base-content/60">
                    {t('common:pagination_info', { page, total: totalPages, count: paginatedProducts.length, label: t('common:items') })}
                </div>
                <div className="join shadow-sm border border-base-300">
                    ...
                    <button className="join-item btn btn-sm bg-base-100 no-animation">
                        {t('common:pagination.page_info_simple', { page, count: totalPages })}
                    </button>
                    <button 
                        className="join-item btn btn-sm btn-outline bg-base-100" 
      ...
      <PremiumModal
        ...
        title={t('reappro.modal_confirm.title')}
        maxWidth="max-w-md"
      ...
      >
        <div className="p-6 text-center">
          ...
          <h3 className="text-xl font-bold text-base-content mb-2">{t('reappro.modal_confirm.question')}</h3>
          <p className="text-base-content/60 text-sm mb-6 leading-relaxed">
            {t('reappro.modal_confirm.message', { count: toProcessAll.length })}
          </p>
      ...
      <SudoValidationModal
        ...
        saving={sudoSaving}
        title={t('reappro.modal_sudo.title')}
        message={t('reappro.modal_sudo.message')}
      />
    </div>
  );
}
