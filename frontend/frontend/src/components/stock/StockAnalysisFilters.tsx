...
export const StockAnalysisFilters: React.FC<StockAnalysisFiltersProps> = ({
    ...
    const tabs = [
        { id: 'unsold', label: t('analyse.tabs.unsold'), icon: <Accessibility className="w-4 h-4" /> },
        { id: 'overstock', label: t('analyse.tabs.overstock'), icon: <TrendingUp className="w-4 h-4" /> },
        { id: 'shortage', label: t('analyse.tabs.shortage'), icon: <AlertTriangle className="w-4 h-4" /> }
    ];
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-widest text-base-content/40 flex items-center gap-1.5 ml-1">
                            <User className="w-3 h-3" /> {t('analyse.filters.supplier')}
                        </label>
                        <select 
                        ...
                        >
                            <option value="">{t('analyse.filters.all_suppliers')}</option>
                            {fournisseurs.map(f => (
                    ...
                    {activeTab === 'unsold' && (
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black uppercase tracking-widest text-base-content/40 flex items-center gap-1.5 ml-1">
                                <Calendar className="w-3 h-3" /> {t('analyse.filters.days_threshold')}
                            </label>
                            <select 
                                className="select select-sm select-bordered w-full font-bold focus:ring-2 focus:ring-primary/20 transition-all"
                                value={unsoldDays}
                            ...
                            >
                                <option value={30}>30 {t('analyse.days')}</option>
                                <option value={60}>60 {t('analyse.days')}</option>
                                <option value={90}>90 {t('analyse.days')}</option>
                                <option value={180}>180 {t('analyse.days')}</option>
                                <option value={365}>365 {t('analyse.days')}</option>
                            </select>
                        </div>
                    )}
                </div>
