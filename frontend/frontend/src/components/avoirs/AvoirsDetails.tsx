            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-base-100 p-4 rounded-2xl shadow-sm border border-base-300 sticky top-4 z-50">
                ...
                <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                    ...
                    {(selectedAvoir.status?.toUpperCase() === 'BROUILLON' || selectedAvoir.status?.toUpperCase() === 'BRO') && (
                        <>
                            <button 
                                className="inline-flex items-center justify-center h-9 px-4 rounded-lg border border-red-200 bg-error/10 text-error text-sm font-bold hover:bg-error/20 transition-all"
                            ...
                            <button 
                                className="inline-flex items-center justify-center gap-2 h-9 px-4 bg-success text-white rounded-lg text-sm font-bold hover:bg-success-focus transition-colors shadow-sm"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="bg-base-100 rounded-2xl shadow-sm border border-base-300 p-5">
                        <h2 className="text-sm font-bold text-base-content/60 uppercase tracking-widest mb-4">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="inline-flex items-center px-2.5 py-1 rounded-md bg-base-200 text-base-content/60 text-sm font-medium border border-base-300">
                            ...
                            {selectedAvoir.observations && (
                                <div>
                                    ...
                                    <p className="text-sm bg-base-200 p-3 rounded-xl border border-base-200/50">
                                        {selectedAvoir.observations}
                                    </p>
                                </div>
                            )}
                    ...
                    <div className="lg:col-span-3 bg-base-100 rounded-2xl shadow-sm border border-base-300 p-4 flex flex-wrap items-center gap-x-8 gap-y-4 text-sm">
                        <div className="flex flex-col">
                            <span className="text-[9px] font-bold text-base-content/50 uppercase leading-none mb-1">{t('stock:avoirs.details.items_count')}</span>
                            <span className="font-mono font-bold text-base-content/60 text-base whitespace-nowrap">
                                {selectedAvoir.produits?.length || 0}
                            </span>
                        </div>
                        
                        <div className="flex flex-col border-l pl-5 border-base-200">
                            <span className="text-[9px] font-bold text-base-content/50 uppercase leading-none mb-1">{t('stock:avoirs.details.total_qty')}</span>
                            <span className="font-mono font-bold text-base-content/60 text-base whitespace-nowrap">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-base-100 rounded-2xl shadow-sm border border-base-300 overflow-hidden">
                        ...
                        <div className="p-4 sm:p-5 border-b border-base-200 bg-base-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            <div>
                                <h3 className="font-bold text-lg flex items-center gap-2">
                                    {t('stock:avoirs.details.lines_title')}
                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-primary/10 text-primary border border-indigo-200">
                                        {selectedAvoir.produits?.length || 0}
                                    </span>
                                </h3>
                            ...
                            <button 
                                ...
                                className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-base-content/70 hover:bg-base-200 rounded-lg text-sm font-medium transition-colors gap-2 w-full sm:w-auto ${allLinesClosed ? 'btn-outline' : 'btn-neutral'}`}
                            >
                                {allLinesClosed ? (
                                    <>
                            ...
                            <table className="w-full text-sm border-separate border-spacing-0">
                                ...
                                <tbody>
                                    {selectedAvoir.produits?.map((ligne, idx) => (
                                        <tr key={ligne.id || idx} className="hover:bg-base-200 transition-colors group">
                                            <td className="text-center">
                                                <button 
                                                    onClick={() => handleToggleCloture(ligne.id, ligne.est_cloture)}
                                                    className={`inline-flex items-center justify-center size-8 rounded-full text-base-content/60 hover:text-base-content hover:bg-base-200 transition-colors ${ligne.est_cloture ? 'text-success hover:bg-success/10' : 'text-base-content/50 hover:bg-base-200'}`}
