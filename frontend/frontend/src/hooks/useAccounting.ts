...
export const useAccounting = () => {
    ...
    const { data: exercices } = useQuery<Exercice[]>({
        ...
        queryFn: async () => {
            ...
            if (data.length > 0 && !currentExercice) {
                // Par défaut, prendre l'exercice qui contient la date du jour
                const today = new Date().toISOString().split('T')[0];
                const active = data.find((ex: Exercice) => today >= ex.date_debut && today <= ex.date_fin) || data[0];
